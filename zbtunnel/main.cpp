					...
					*out_ << "Loading conf: " << filename << "\n";
					...
					BOOST_FOREACH(ptree::ptree::value_type node, pt) {
						...
						if (node.first.compare("global") == 0) {
							...
							gconf.log_level((ZbConfig::log_level_type)global.get("log_level", (int)gconf.log_level()));
							...
							boost::shared_ptr<ZbTunnel> t(new ZbIoTunnel(node.first));
							...
							boost::shared_ptr<ZbTunnel> t(new ZbSocketTunnel(node.first));
							tunnels_[node.first] = t;
							t->start_with_config(node.second);
							boost::thread* tr = new boost::thread(t->get_worker_func());
							tg_.add_thread(tr);
...
int main(int argc, char **argv) {
	return zb::tunnel::ZbTunnelMain().run(argc, argv);
}
